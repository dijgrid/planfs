...
- [ ] Current repositories have an unambiguous inferred or explicit format version
- [ ] Schemas expose stable version identifiers independent of package version
- [ ] Unsupported newer versions fail with actionable compatibility guidance
- [ ] `planfs migrate` or equivalent previews all changes without writing by default
- [ ] Migration application validates the complete repository and avoids partial writes
- [ ] Normal saves do not silently migrate unrelated artifacts
- [ ] Documentation replaces unsupported versioning claims with the implemented contract
- [ ] Tests cover current, older supported, newer unsupported, no-op, failed, and successful migrations
